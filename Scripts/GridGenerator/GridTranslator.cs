...
{
    public static void TranslateAndPlace(Node3D parent, GridGenerator gridGenerator, GridTheme gridTheme, Vector2 cellSize)
    {
        var translationOutput = Translate(gridGenerator, gridTheme, cellSize);
    ...
    public static Dictionary<Vector3, PackedScene> Translate(GridGenerator gridGenerator, GridTheme gridTheme, Vector2 cellSize)
    {
        ...
        for (uint x = 0; x < gridGenerator.SizeX; x++)
            ...
            {
                var position = new Vector3(x * cellSize.X, 0, y * cellSize.Y);
